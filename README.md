...
- 已把文章列表页、详情页和首页最新动态统一到同一份 Markdown frontmatter 内容源。
...
- `src/data`: 页面固定文案和静态配置。
- `src/content/posts`: 统一文章内容源，使用 Markdown frontmatter 保存文章元数据与正文。
...
- 当前文章元数据与正文已经统一进 `src/content/posts/*.md`。
- `frontmatter` 现在还统一承载 `author`、`coverRatio`、`series` 和 `featured`，让列表页、详情页与首页推荐共享同一套文章元信息。
- 后续可以继续从现有数据层扩展成真实 API，或直接在 `src/content/posts` 中新增 Markdown 文章。
